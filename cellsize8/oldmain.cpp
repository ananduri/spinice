...
int main(int argc, char *argv[]) 
{
	...
	int step = atoi(argv[2]); //number of separate runs, useless now
	...
	else//or read in from file
	{
		FILE* istream;
		char iname[100];
		sprintf(iname,"/home/ananduri/kmc/samples/a%d/spin_T%.2f_S%d_a%d_lab%d_step%d.bin",cellsize,T,S,cellsize,label,step-1);
	...
	for (double temp=10;temp>=2;temp-=dT)
	{
		...
		evolvesave(timestate,intmat,temp,RanGen_mersenne,S,label);

		//save
		sprintf(tname,"/home/ananduri/kmc/samples/a%d/spin_T%.2f_S%d_a%d_lab%d_step%d.bin",cellsize,temp,S,cellsize,label,step);	
